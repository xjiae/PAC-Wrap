...
def run_devnet(args):
    ...
    for nm in names:
        ...
        fnrs = np.zeros(runs)  
        fprs = np.zeros(runs)
        fnrs_origin = np.zeros(runs)  
        fprs_origin = np.zeros(runs)  
        filename = nm.strip()
        global data_format
        ...
        for i in np.arange(runs):  
            ...
            # Our method
            # trainsize = int(0.9*x.shape[0])
            # length = x.shape[0]
            # normalDataX = x[labels==0,:]
            # normalDatay = labels[labels==0].to_numpy()
            # abnormalDataX = x[labels==1, :]
            # abnormalDatay = labels[labels==1].to_numpy()

            # indices = np.arange(normalDataX.shape[0])
            # np.random.shuffle(indices)
            # indices_normal = indices[:int(normalDataX.shape[0] * 0.8)]
            # x_train = np.vstack([normalDataX[indices_normal], abnormalDataX[:10]])
            # y_train = np.hstack([normalDatay[indices_normal], abnormalDatay[:10]])

            # indices = np.arange(normalDataX.shape[0])
            # np.random.shuffle(indices)
            # indices_abnormal = indices[:int(normalDataX.shape[0] * 0.2)]

            # x_test = np.vstack([normalDataX[indices_abnormal], abnormalDataX[10:]])
            # y_test = np.hstack([normalDatay[indices_abnormal], abnormalDatay[10:]])
            ...
            scores = iid.sigmoid(scores)
            y_test = y_test.to_numpy()
            probas_normalDataX = scores[y_test==0,].squeeze()
            normalDatay = y_test[y_test==0,]
            probas_abnormalDataX = scores[y_test==1,].squeeze()
            abnormalDatay = y_test[y_test==1.]
            epsilon = 0.05
            delta = 0.05
            testX, testy, [valX_5, valX_75, valX], [valy_5, valy_75, valy] = iid.split_data(probas_normalDataX, 
                                                                              ...
                                                                              abnormalDatay,
                                                                              testsize=scores.shape[0],
                                                                              valsize=scores.shape[0],
                                                                              synth=False)
            T_best_fp, T_best_fn, frac, fpr, fnr = iid.evaluate(testX, testy, valX, valy, epsilon, delta)
            plt.boxplot(x=[probas_normalDataX, probas_abnormalDataX], positions = [1,2], labels=['normal', 'abnormal'] )
            plt.axhline(y = T_best_fn, color = 'r', linestyle = ':', label="T_fnr_origin")
            plt.axhline(y = T_best_fp, color = 'g', linestyle = ':', label="T_fpr_origin")
            fnrs[i] = fnr
            fprs[i] = fpr
            tau, T_fn, T_fp, epsilon_p = iid.strategy(T_best_fn, T_best_fp, testX, testy, valX, valy, epsilon, delta)
            fpr, fnr, error = iid.evaluate_tau(testX, testy, tau)
            # plt.rcParams.update({'font.size':20})
            plt.boxplot(x=[probas_normalDataX, probas_abnormalDataX], positions = [1,2], labels=['normal', 'abnormal'] )
            plt.axhline(y = T_fn, color = 'r', linestyle = '-', label=r"$\hat \tau_{fn}$")
            plt.axhline(y = T_fp, color = 'g', linestyle = '-', label=r"$\hat \tau_{fn}$")
            plt.axhline(y = tau, color = 'b', linestyle = '--', label=r"$\tau$")
            plt.title(rf'$\epsilon$: {epsilon_p}, $\tau$: {tau}, $\hat \epsilon$: {error}')
            plt.legend()
            plt.savefig(f'{nm}_Box_DevNet.png')
            test_time += time.time() - start_time
            rauc[i], ap[i], fnrs_origin[i], fprs_origin[i] = aucPerformance(scores, y_test)   
            pdb.set_trace() 
        ...
        print("average AUC-ROC: %.4f, average AUC-PR: %.4f, average fnr: %.4f, average fpr: %.4f" % (mean_auc, mean_aucpr, mean_fnr, mean_fpr))    
...
parser.add_argument("--data_set", type=str, default='bank-additional-full_normalised', help="a list of data set names")
