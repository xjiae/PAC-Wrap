...
from deviation_network.utils import dataLoading, aucPerformance, writeResults, get_data_from_svmlight_file
...
def run_devnet(args):
    ...
    # names = ['census-income-full-mixed-binarized']
    # names = ['celeba_baldvsnonbald_normalised']
    # names = ['bank-additional-full_normalised']
    # names = ['KDD2014_donors_10feat_nomissing_normalised']
    ...
    for nm in names:
        ...
        for i in np.arange(runs):  
            ...
            model = deviation_network(input_shape, network_depth)
            print(model.summary())  
            model_name = "./models/devnet_"  + filename + "_" + str(args.cont_rate) + "cr_"  + str(args.batch_size) +"bs_" + str(args.known_outliers) + "ko_" + str(network_depth) +"d.h5"
            ...
            plt.title(r'$\epsilon$: {:.3f}, $\tau$: {:.3f}, ERR: {:.3f}'.format(epsilon_p, tau, error))
            ...
            print('Best threshold: ', thresholds[np.nanargmax(f1_scores)])
            print('Best F1-Score: ', np.nanmax(f1_scores))
            fprs_origin[i], fnrs_origin[i], error = iid.evaluate_tau(testX, testy, threshold)
            break
